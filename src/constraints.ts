...
export class EarthLimbConstraint extends Constraint {
  ...
  constructor({
    ...
    if (!Number.isFinite(elevation)) {
      throw new Error('Invalid elevation: the elevation must be finite')
    }

    if (elevation < 0) {
      throw new Error('Invalid elevation: the elevation must not be negative')
    }

    if (!Number.isFinite(grazing)) {
      throw new Error('Invalid grazing height: the grazing height must be finite')
    }

    // N.B. A negative grazing height shrinks the shell within the Earth, and so it would occult a
    // smaller cone than the Earth itself does, e.g., it would report an occulted target as one that
    // is observable:
    if (grazing < 0) {
      throw new Error('Invalid grazing height: the grazing height must not be negative')
    }

    if (!Number.isFinite(clearance)) {
      throw new Error('Invalid clearance: the clearance must be finite')
    }

    if (clearance <= 0) {
      throw new Error('Invalid clearance: the clearance must be greater than zero')
    }
