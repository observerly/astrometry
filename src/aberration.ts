...
import { convertCartesianToEquatorial, convertEquatorialToCartesian } from './coordinates'
...
export const getCorrectionToEquatorialForLightDeflection = (
  ...
  // The Schwarzschild radius of the Sun, e.g., 2GM☉/c² for the TDB-compatible heliocentric gravitational constant
  // of the IAU 2009 system of astronomical constants (in astronomical units):
  ...
  // Get the difference in fractional Julian centuries between the target date and J2000.0, at the Terrestrial
  // Time of the given date, which the general precession in longitude is referred to:
  ...
  // equinox of the date from the mean equinox of J2000.0 along the ecliptic, by which the geometric longitude of
  // the Sun, referred to the equinox of the date, is referred back to the equinox of J2000.0 (in degrees):
  const pA =
    (5028.796195 * T +
      1.1054348 * T ** 2 +
      0.00007964 * T ** 3 -
  ...
  // The unit vector of the Sun in the ecliptic frame, from its geometric longitude and latitude referred to the
  // equinox of J2000.0, e.g., the spherical to cartesian conversion of the ecliptic coordinate, which is that of
  // the equatorial coordinate with the longitude and latitude in place of the right ascension and declination.
  //
  // N.B. The ecliptic of the date is taken as the ecliptic of J2000.0, the precession of the ecliptic itself being
  // ~47 arcseconds per century, which displaces the direction of the Sun by that much, e.g., by a ~0.0001
  // arcsecond of deflection at the limb of the Sun per century:
  const ecliptic = convertEquatorialToCartesian({ ra: sun.λ - pA, dec: sun.β })

  // Get the mean obliquity of the ecliptic at J2000.0 of IAU 2006 (in degrees):
  const ε = 84381.406 / 3600

  // The unit vector of the Sun, rotated from the ecliptic frame into the equatorial frame of J2000.0, e.g., by the
  // passive rotation of the frame about the x axis by the negative of the obliquity of the ecliptic:
  const equatorial = getRotatedCartesianCoordinate(getRotationMatrix('x', -ε), ecliptic)

  // Get the distance of the Sun from the Earth (in astronomical units):
  const d = sun.R / AU_IN_METERS

  // The unit vector of the target:
  const p = convertEquatorialToCartesian(target)

  // The unit vector from the Sun to the Earth, e.g., the direction of the Sun from the Earth reversed:
  const e = {
    x: -equatorial.x,
    y: -equatorial.y,
    z: -equatorial.z
  ...
  const { ra, dec } = convertCartesianToEquatorial(apparent)

  // The displacement in right ascension, taken the shorter of the two ways about the sphere:
  return {
    ra: getNormalizedAzimuthalDegree(ra - target.ra + 180) - 180,
    dec: dec - target.dec
  }
}
